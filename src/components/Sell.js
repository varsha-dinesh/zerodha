...
function Sell( { price }) {
  ...
  return (
    <>
      {isVisible ?
        <Tooltip label='Sell(S)' fontSize='1.3rem' placement='top' p='1rem' hasArrow='true' borderRadius='5px'>
          <Button colorScheme='red' px='1.3rem' onClick={() => setIsOpen(true)}>S</Button>
        </Tooltip> :
        <Flex colorScheme='gray' px='1.3rem' onClick={openModal} width='100%' mb='2rem' py='1rem'>
      ...
      >
        ...
        <Tabs size='lg' variant='line'>
          ...
          <TabPanels>
            <TabPanel>
              <SellRegular onClose={() => setIsOpen(false)} price={price}/>
            </TabPanel>
          </TabPanels>
        </Tabs>
      </Modal>
